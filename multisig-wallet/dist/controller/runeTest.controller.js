...
const etchingRuneToken = (runeName, runeAmount, runeSymbol, initialPrice, creatorAddress) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (repeatedName)
            return "Already registered in DB";
        ...
        const networkRepeatance = (yield axios_1.default.get(url)).data;
        if (networkRepeatance.data)
            return "Already existed Rune Name";
    ...
    <body style="margin: 0px">
      <div style="color: white; font-size: larger">${name}</div>
    </body>
  </html>`;
        ...
        const generateInscribePSBT = yield (0, psbt_service_2.inscribeRunePSBT)(utxo, script_p2tr, etching_p2tr, etching_redeem, 
        //   wallet.address,
        config_1.ADMIN_ADDRESS, runeSymbol, runeAmount, originalName, spacers);
        const payload = {
            txId: "",
...
const checkTxStatus = () => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        for (const checkRuneEtching of checkRuneEtchingList) {
            ...
            console.log("currentBlockHeight >= checkRuneEtching.blockHeight + 5 ==> ", currentBlockHeight >= checkRuneEtching.block_height + 5);
            if (checkRuneEtching.confirmed &&
                currentBlockHeight >= checkRuneEtching.block_height + 5) {
