...
import { OPENAPI_UNISAT_TOKEN, OPENAPI_UNISAT_URL, OPENAPI_URL, TEST_MODE, WIF_KEY } from "../config/config";
...
export const fetchRuneHolderList = async (runeId: string) => {
  const url = `${OPENAPI_UNISAT_URL}/v1/indexer/runes/${runeId}/holders`;
  const config = {
    headers: {
...
export const getRuneInfo = async (runeId: string) => {
  const url = `${OPENAPI_UNISAT_URL}/v1/indexer/runes/${runeId}/info`;
  const config = {
    headers: {
      Authorization: `Bearer ${OPENAPI_UNISAT_TOKEN}`,
    },
